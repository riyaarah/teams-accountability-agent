...
export default function Home() {
  ...
  function loadSample() { setInput({ ...initialInput, transcript: sampleTranscript }); }
  ...
  return (
    <div style={{ minHeight: "100vh" }}>
      {/* Nav */}
      <nav style={{ borderBottom: "1px solid #2e2e3f", background: "rgba(9,9,12,0.85)", backdropFilter: "blur(20px)", position: "sticky", top: 0, zIndex: 50, padding: "0 2rem", display: "flex", alignItems: "center", justifyContent: "space-between", height: "56px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
          <div style={{ width: "28px", height: "28px", borderRadius: "7px", background: "linear-gradient(135deg,#c9a84c,#a07830)", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <Bot size={15} color="#09090c" />
          </div>
          <span className="font-display" style={{ fontWeight: 700, fontSize: "15px", color: "#f2f0eb", letterSpacing: "0.01em" }}>Accountability Agent</span>
          <span style={{ marginLeft: "8px", padding: "2px 8px", borderRadius: "4px", background: "#1a1a24", border: "1px solid #2e2e3f", fontSize: "11px", fontWeight: 600, color: "#8a6d2e", letterSpacing: "0.06em", textTransform: "uppercase" }}>MS Teams</span>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
          {/* Tabs */}
          {(["analyze", "history", "leaderboard"] as const).map(tab => (
            <button key={tab} type="button" onClick={() => setActiveTab(tab)} style={{
              padding: "5px 14px", borderRadius: "6px", border: "none", cursor: "pointer",
              background: activeTab === tab ? "rgba(201,168,76,0.12)" : "transparent",
              color: activeTab === tab ? "#c9a84c" : "#65625a",
              fontSize: "13px", fontWeight: 600, textTransform: "capitalize",
              borderBottom: activeTab === tab ? "2px solid #c9a84c" : "2px solid transparent",
              transition: "all 0.15s"
            }}>
              {tab === "analyze" ? "Analyze" : tab === "history" ? `History (${history.length})` : "Leaderboard"}
            </button>
          ))}
          <StatusDot label="AI Ready" />
          <StatusDot label="Work IQ" />
        </div>
      </nav>
          ...
          <>
            <section style={{ maxWidth: "1280px", margin: "0 auto", padding: "4rem 2rem 2rem" }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "5rem", alignItems: "start" }}>
                <div>
                  <div style={{ display: "inline-flex", alignItems: "center", gap: "8px", padding: "6px 14px", borderRadius: "100px", border: "1px solid #8a6d2e", background: "rgba(201,168,76,0.06)", marginBottom: "1.5rem" }}>
                    <Zap size={13} color="#c9a84c" />
                    <span style={{ fontSize: "12px", fontWeight: 600, color: "#c9a84c", letterSpacing: "0.06em", textTransform: "uppercase" }}>Multi-agent pipeline · Work IQ</span>
                  </div>
                  <h1 className="font-display" style={{ fontSize: "clamp(2.5rem,5vw,4.5rem)", fontWeight: 800, lineHeight: 0.95, color: "#f2f0eb", margin: "0 0 1.25rem", letterSpacing: "-0.02em" }}>
                    Turn meetings<br />
                    <span style={{ background: "linear-gradient(135deg,#c9a84c,#f0d080)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>into ownership.</span>
                  </h1>
            ...
            <section style={{ maxWidth: "1280px", margin: "0 auto", padding: "0 2rem 4rem" }}>
              ...
              {error && <div style={{ background: "#3d1a1a", border: "1px solid #7a2020", borderRadius: "12px", padding: "1rem 1.25rem", color: "#e05252", fontWeight: 600 }}>{error}</div>}
...
function StatusDot({ label }: { label: string }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: "6px", padding: "4px 10px", borderRadius: "100px", background: "#111118", border: "1px solid #2e2e3f" }}>
...
function Pill({ icon, label, gold }: { icon: React.ReactNode; label: string; gold?: boolean }) {
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: "6px", padding: "6px 14px", borderRadius: "100px", border: `1px solid ${gold ? "#8a6d2e" : "#2e2e3f"}`, background: gold ? "rgba(201,168,76,0.08)" : "#1a1a24", fontSize: "12px", fontWeight: 600, color: gold ? "#c9a84c" : "#a8a499", letterSpacing: "0.04em" }}>
      {icon}{label}
    </span>
  );
}
...
function FeatureCard({ icon, title, copy }: { icon: React.ReactNode; title: string; copy: string }) {
  return (
    <div style={{ background: "#111118", border: "1px solid #2e2e3f", borderRadius: "10px", padding: "1rem" }}>
...
function LoadingState() {
  return (
    <div style={{ background: "#111118", border: "1px solid #2e2e3f", borderRadius: "12px", padding: "2rem", display: "flex", alignItems: "center", gap: "1.5rem" }}>
      <div style={{ display: "flex", gap: "6px" }}>
        {[0,1,2].map(i => <div key={i} className="loading-dot" style={{ width: "8px", height: "8px", borderRadius: "50%", background: "#c9a84c", animationDelay: `${i * 0.2}s` }} />)}
      </div>
      <div>
